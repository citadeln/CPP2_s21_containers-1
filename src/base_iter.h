...
class BaseIter {
 public:
  ...
  BaseIter(iterator_ptr ptr_) { this->ptr = ptr_; };
  // Pre-increment
  BaseIter operator++() {
    this->ptr++;
    return this->ptr;
...
// class BaseIter<T, s21::enable_if< s21::list|| s21::map || s21::set ||
// s21::vector ||s21::array>{
template <typename T, template <typename> class childIter>
...
class VectorIterator : public BaseIterEnd<T, VectorIterator> {
 public:
  ...
  VectorIterator(iterator_ptr ptr_) { this->ptr = ptr_; };
};
template <typename T>
class ArrayIterator : public BaseIterEnd<T, ArrayIterator> {
 public:
  ...
  ArrayIterator(iterator_ptr ptr_) { this->ptr = ptr_; };
};

// class BaseIter<T, s21::enable_if< s21::list|| s21::map || s21::set ||
// s21::vector ||s21::array>{ template<typename T> class ListIterator : public
// BaseIterMiddle<T>{};
